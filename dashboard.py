...
def train_model(model_name, hyperparameters, features, target):
...
def predict(model_id, features):
    ...
    try:
        response = requests.post(f"{API_URL}/predict/{model_id}", json=data)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
def delete_model(model_id):
    ...
    try:
        response = requests.delete(f"{API_URL}/trained-models/{model_id}")
        response.raise_for_status()
        return True
    except requests.exceptions.RequestException as e:
...
def retrain_model(model_id, hyperparameters, features, target):
    ...
    try:
        response = requests.put(f"{API_URL}/retrain/{model_id}", json=data)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        ...
        try:
            ...
            if st.button("Начать переобучение"):
                with st.spinner("Модель переобучается..."):
                    ...
                    result = retrain_model(model_id_to_retrain, hyperparams, features, target)
    ...
    try:
        ...
        if st.button("Начать обучение"):
            with st.spinner("Модель обучается..."):
                ...
                result = train_model(model_name, hyperparams, features, target)
            if result:
        ...
        try:
            ...
            if st.button("Получить предсказания"):
                with st.spinner("Получение предсказаний..."):
                    result = predict(selected_model_id, features)
